...
◊(define-meta tags "siri,apple,iphone")
...
Someone’s bound to respond that Siri is still officially beta software. I am aware of that, and don’t expect it to be perfect◊link["#sirifn:2"]{2}.

But it’s also worth noting that Apple, like most software companies, has introduced many apps and features that they then failed to improve upon or even abandoned◊link["#sirifn:3"]{3}. My gut feeling is that Siri is in danger of ending up as the next Mobile Me, especially given that it has received almost no improvement or attention since its release so far.

◊hr{}

◊ol{
◊li{Connection failures are especially common when you are on the outer edge of a known wifi network, such as when in the driveway or parking lot, or walking to/from your house. The iPhone seems to think it is connected via wifi for some time after you have left range, causing Siri to attempt using a nearly non-existent connection. ◊link["#fnref:1"]{↩}}
◊li{Although I do expect beta software to be at least feature-complete, which I don’t believe Siri is. ◊link["#fnref:2"]{↩}}
◊li{See the Calendar, Weather, Music and Stock apps, iTunes Genius, Ping, and of course Mobile Me. ◊link["#fnref:3"]{↩}}
}
