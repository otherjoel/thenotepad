...
I became interested in ◊link["http://en.wikipedia.org/wiki/LaTeX"]{LaTeX} out of a desire to be able to produce high-quality PDFs for self-published books. Someday I hope to be able to produce books of comparable quality to ◊link["http://www.tsengbooks.com/"]{these humanities books} typeset in TeX. This idea became even more feasible when I discovered the text content could be written in Markdown and converted to LaTeX with ◊link["http://johnmacfarlane.net/pandoc/index.html"]{◊code{pandoc}} (More information in ◊link["http://www.charlietanksley.net/philtex/primarily-pandoc/"]{this article}).
...
◊li{Add ◊code{\usepackage{xltxtra}} at the beginning of your preamble (some XeTeX goodies, in particular it also loads fontspec, which is needed for font selection).}
