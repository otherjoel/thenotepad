...
◊item{◊strong{Use the Internet from behind a router.} Don’t plug directly into your Cable or ◊span[#:class "caps"]{DSL} modem. Plug a router into that thing (a wireless router if that’s how you like to roll) and then use that. Most off-the-shelf routers have built-in firewalls that prevent the most common attacks.}
