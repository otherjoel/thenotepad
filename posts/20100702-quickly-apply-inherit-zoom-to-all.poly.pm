...
I often compile ◊span[#:class "caps"]{PDF} reports totaling 1,000 to 2,000 pages, and am always creating hierarchical bookmarks for these files. One of my beefs is that often, clicking a bookmark will change the magnification setting to “fit width” from whatever you had it set at. Ideally, clicking a bookmark should default to just leaving the zoom level at whatever the reader is currently set to use (“Inherit zoom”). I’ve found no way in Acrobat to set the default behaviour for newly-created bookmarks, and changing these links one at a time in Acrobat is extremely tedious.

Thankfully, there is ◊link["http://maba.wordpress.com/2004/06/20/pdf-bookmark-hacks/"]{a solution} provided by Martin Backschat:

◊blockquote{
To modify this annoyance you can directly modify the ◊span[#:class "caps"]{PDF} document in your Text editor.

With UltraEdit, for example, I load the ◊span[#:class "caps"]{PDF} document and open the “Search and Replace” box, enable “Regular Expressions” and replace all occurrences of “R/XYZ*]” with “R/XYZ]”, and then also all occurrences of “R/Fit*]” with “R/XYZ]”. Now safe the document.
...
I don’t have UltraEdit but I was able to make this work using ◊link["http://sourceforge.net/projects/notepad-plus/"]{Notepad++}. The exact search/replace text was slightly different for my ◊span[#:class "caps"]{PDF} files (note the added space) but the principle is the same:
...
◊li{Use ◊code{"R /XYZ(.*)]"} (without quotes) as your search string and ◊code{"R XYZ]"} as your replace string and replace all instances.}
...
<span class="cite" data-citation-ids="Joshua">@Joshua</span>, thanks a lot. I did use JPDFBookmarks. Really nice app.
